...
export async function handlePublic(request: Request, env: Env): Promise<Response> {
  ...
  if (path === "/" && request.method === "GET") return page("SMPT Mail Plugin", "<h1>SMPT Mail Plugin</h1><p>Use your own IMAP and SMTP mailbox in ChatGPT and Codex. Connect the plugin, then enter your email address, IMAP and SMTP hosts, and an app password on this site.</p><p>The MCP endpoint is <code>/mail</code>.</p><p><a href='/privacy'>Privacy</a> · <a href='/terms'>Terms</a> · <a href='/support'>Support</a></p>");
  if (path === "/privacy") return page("Privacy", "<h1>Privacy</h1><p>SMPT Mail Plugin stores your mailbox address, IMAP and SMTP hostnames, and an AES-GCM encrypted app password in Cloudflare D1. The encryption key is stored separately as a Cloudflare Worker secret. We use this data only to connect to your email provider when you request a tool action. Message content is returned to your MCP client and is not saved in our database.</p><p>OAuth grants and short-lived connection and abuse prevention records are stored in Cloudflare KV. Daily send counters are stored in D1 for up to 30 days. The <code>remove_mailbox</code> tool deletes your saved mailbox details, encrypted password, and send counters. Disconnect the MCP client separately to revoke its OAuth token.</p><p>Do not enter a password in chat or a public issue. For support, see <a href='/support'>Support</a>.</p>");
  if (path === "/terms") return page("Terms", "<h1>Terms</h1><p>Use SMPT Mail Plugin only with a mailbox you control. You are responsible for messages you choose to send and for following your email provider's rules. Spam, unlawful use, and attempts to access another person's mailbox are prohibited. The service is provided as-is without a delivery guarantee. You may disconnect your mailbox at any time using the <code>remove_mailbox</code> tool.</p><p>See <a href='/privacy'>Privacy</a> and <a href='/support'>Support</a>.</p>");
  if (path === "/support") return page("Support", "<h1>Support</h1><p>For help, report a bug, or request data deletion support, open an issue in the <a href='https://github.com/javadfaghih/softlian-mail-mcp/issues'>SMPT Mail Plugin repository</a>. Never put a mailbox address, app password, or message content in a public issue.</p>");
